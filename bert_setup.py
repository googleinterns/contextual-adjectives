...
class Bert:
    ...
    def perturb_bert(self, sentence, num_suggest, masked_index):
        ...
        num_suggest : Number of new words need to be suggested.
        ...
        # For each idx, use Bert to generate k (i.e., num_suggest) candidate tokens
        ...
        # Error when invalid index tokens.    
        except KeyError:
            return {}

        # Get the similar words
        topk_idx = torch.topk(prediction[0, masked_index], num_suggest)[1].tolist()
        scores = torch.topk(prediction[0, masked_index], num_suggest)[0].tolist()
